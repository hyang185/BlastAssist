...
from BlastAssist.torch.torchnn import Classifier


class Resnet101TransferLearned(Classifier):
    def __init__(self, input_size=(200, 200), num_classes=4, pretrained=True):
        """
...
class Resnet50TransferLearned(Classifier):
    def __init__(self, input_size=(200, 200), num_classes=4, pretrained=True):
        """
        Explain why input_size is here to be helpful.
        """
        self.pretrained = pretrained
        super(Resnet50TransferLearned, self).__init__(
            input_size=input_size, num_classes=num_classes)

    def _initialize(self):
        r50 = torchvision.models.segmentation.fcn_resnet50(
            pretrained=self.pretrained)
        r50.classifier[4] = torch.nn.Conv2d(
            512, self.num_classes, kernel_size=(1, 1), stride=(1, 1))
        self.resnet = r50
